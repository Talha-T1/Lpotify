...
export default function Player() {
  ...
  if (!currentTrack) {
    return (
      <div className="h-[90px] bg-lpotify-dark border-t border-lpotify-gray/30 flex items-center justify-center shrink-0">
        <p className="text-lpotify-gray-light text-sm">{t('chooseSong')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-[90px] bg-lpotify-dark border-t border-lpotify-gray/30 px-4 flex items-center justify-between shrink-0">
      {/* Track Info */}
      <div className="flex items-center gap-3 w-[30%] min-w-[180px]">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-lpotify-gray-light truncate">{currentTrack.artist}</p>
        </div>
        <button onClick={toggleLike}
          className={`ml-2 p-1 rounded-full transition-colors ${isLiked ? 'text-lpotify-green' : 'text-lpotify-gray-light hover:text-white'}`}>
      ...
      <div className="flex flex-col items-center w-[40%] max-w-[722px]">
        ...
        <div className="flex items-center gap-2 w-full">
          <span className="text-[11px] text-lpotify-gray-light w-10 text-right">{formatDuration(progress)}</span>
          ...
          <span className="text-[11px] text-lpotify-gray-light w-10">{formatDuration(duration)}</span>
        </div>
      </div>
